...
We are going to design a map of worst 100 NYC rental properties in terms of housing violations. The map will color the points by the average number of violations and size the points by the number of units in the building. 
...
3. Call `addProviderTiles()` to load up provider tiles that constitute a base map. A number of different map providers have provider tiles that we can reference here. A few examples of the arguments we can supply to this function include:
...
map1 <- leaflet(width = "100%") |>
  setView(lng = 0.00, lat = 0.00, zoom = 0) |>
  addProviderTiles(providers$Stamen.Toner)

map1
```

## Ex 1: Intitializing the NYC Map

Adjust the code below to center the map on NYC. You’ll need to look up the coordinates for NYC using the `geocode_OSM()` function in your console as we did in Lec30-32 In-class Exercises. Adjust the `zoom` to keep the whole city in view (setting the zoom level between 1 and 16). When you are happy with the View, switch out the provider tiles to a base map that won’t distract from the data points we will layer on top of this map. 
...
# Setting Geometry and Transform Data CRS

We will be looking to convert our data into an object with coordinate-based geometry so that we can map it. We can use the `st_as_sf()` function to do this. 
...
## Ex 2: Converting Data Frames to `sf` Objects
...
> Hint: You can refer to Lec30-32 In-class Exercises Section 1: Converting data frames to sf objects.
...
#worst_buildings_sf <- worst_buildings |>
...
## Ex 4: Styling the Map

Map isn’t so legible/beautiful at this point, right? Copy the map you just created into the code chunk below. Add some arguments in the `addCircleMarkers()` function as we did in Lec30-32 In-class Exercises to help with the map’s legibility, for example, you should adjust the radius, weight, color, fillColor, and fillOpacity, and understand how each of these arguments will change the style of the map. For now you can set the color to “black” and the fillColor to “white”. 
...
Now that our map is looking more legible, let’s color the circles by the 2021 average open violations at each property on the map. Remembering back to our lesson on Visualization Aesthetics in Lab 2, we should keep in mind that `avg_violations_weighted` is a numeric variable, and therefore, we will create a sequential color palette to map it.

> Hint: You can refer to Lec30-32 In-class Exercises Section 3: Interactive maps using leaflet, in particular, colors in leaflet.

## Ex 5: Creating Three Color Palettes

Create three palettes below (one using each function `colorNumeric()`, `colorBin()`, and `colorQuantile()`), setting the palette to “YlOrRd”, and the domain to `worst_buildings_sf$avg_violations_weighted`. For `colorBin()`, set the `bins` to 4, and for `colorQuantile()`, set `n` to 4.

```{r}
pal_num <- colorNumeric(palette="YlOrRd", 
                        domain = worst_buildings_sf$avg_violations_weighted)
...
## Ex 7: Understanding the Three Color Palettes

**Discussion**: Why do the colors appear differently on each map? Which map best represents the distribution of values in avg_violations_weighted?

**Answer**:



## Ex 8: Sizing the Circles
...
## Ex 9: Adding Legends and Labels
...
2. Add a pipe to the end of the `addCircleMarkers()` and then add the function `addLegend()` for the meaning of the colors represented on the map. For example, you will need an argument for `position`, `pal`, `values`, and `title`.
...
## Ex 10: Weekly Reflections

Fill out this week and last week's reflections Google form to receive credit.
