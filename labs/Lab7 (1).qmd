...
# Step 1: Filter the `seattle_people_moving` to the rows where age groups were counted.
...
# Step 2: Select `unique_moving_id` to `moving_time_of_day`, and `moving_0_4` to `moving_25_64`.

```{r}
# Paste the code below and add your code

```

# Step 3: Pivot all age columns from wide to long format with two new columns: age_group and count.
...
# Step 4: Recode the values in the `age_group` column to use dashes instead of underscores.
...
# Step 5: Create a histogram plotting the distributions of people counts in each observational period of the study, faceted by age group.
...
# Bonus! Factor the `age_group` variable so that the faceted plots appear in order by age_group. 

> Hint: You can call the `factor()` function in `mutate()`
...
Now we filter to group_size.

# Step 6: Filter the `seattle_people_moving` to the rows where group_size was counted.
...
# Step 7: Group by the `location_id`, `moving_time_of_day`, `moving_microclimate` and then summarize the average of `moving_row_total` in a column called `avg_moving_row_total`.
...
# Step 8: Pivot `moving_microclimate` and `avg_moving_row_total` from long to wide format. Be sure to repair names!
