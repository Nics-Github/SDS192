...
# Write your code below
Schwab_locs <- tibble(
  address=c("UC Santa Cruz", "San Leandro High School", "Appalachian State University", "University of Texas at Austin")) |>
  geocode(address, method = "osm") |>
  mutate(
    name = c("UC Santa Cruz", "San Leandro High School", "Appalachian State University", "University of Texas at Austin")
)

leaflet() |>
  addTiles()|>
  addMarkers(data= Schwab_locs, lng = ~long,lat = ~lat, popup = ~name)
```
...
leaflet() |>
  addTiles()|>
  addCircles(data= Schwab_locs, lng = ~long,lat = ~lat, popup = ~name) 
```
