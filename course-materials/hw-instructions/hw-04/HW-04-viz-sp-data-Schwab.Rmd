---
title: "HW - 04 - La Quinta is Spanish for next to Denny's"
...
The inspiration for this HW comes from a blog post by John Reiser on his *new jersey geographer* blog.
...
In this HW we focus on visualization and analysis of these data.
...
We'll use the **tidyverse** package for much of the data wrangling and visualisation and the data lives in the folder data.
...
Note that these data were scraped from [here](https://locations.dennys.com/) and [here](https://www.lq.com/en/findandbook/hotel-listings.html), respectively.We'll learn to scrape information from the web soon

Because the data sets do not come from a package we cannot use ?dennys or ? laquinta. 
You can find information on them [here](https://rstudio-education.github.io/dsbox/reference/dennys.html) and [here](https://rstudio-education.github.io/dsbox/reference/laquinta.html).
...
laquinta <- laquinta %>%
  mutate(country = case_when(
    ...
    state == "ANT"           ~ "Colombia",
    state %in% c("AG","CI","VE","PU","SL", "CH") ~"Mexico",
    #TRUE ~ state
    # fill in the rest
  ))
```
...
#California has the most Dennys
dennys |>
  count(state,sort=TRUE)
```    
...
```{r}
dennys_per_thou<- dennys %>%
  count(state) %>%
  inner_join(states, by = c("state"="abbreviation"))|>
  mutate(area= area/1000) |>
  mutate(dennys_per_thou_miles = n/area)

laquinta_per_thou <- laquinta %>%
  count(state) %>%
  inner_join(states, by = c("state"="abbreviation"))|>
  mutate(area= area/1000) |>
  mutate(laquintas_per_thou_miles = n/area)
...
```{r}
dennys_per_thou |>
  arrange(desc(by_group = dennys_per_thou_miles))

laquinta_per_thou |>
  arrange(desc(by_group = laquintas_per_thou_miles))
