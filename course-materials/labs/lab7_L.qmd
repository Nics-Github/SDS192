...
#View(Monarch_Watch_Tag_Recoveries_Central_Mexico_mexico_recoveries)
...
butterflies <- top_7_butterfly |>
  st_as_sf(coords = c("long", "lat"), crs = 4326)
ggplot() +
  geom_sf(data =USA_map) +
  labs(title="Top 7 U.S. Butterflies") +
  geom_jitter()+
  geom_sf(data = butterflies, color="orange")
```
...
butterfly_location_summary <- butterfly_location_data |>
   ...
   summarise(
     total_sightings_state = n(),  # Total sightings per state
     total_sightings_2022 = nrow(butterfly_location_data),  # Total sightings in 2022
     proportion = total_sightings_state / total_sightings_2022  # Proportion for each state
   )
...
#First join
butterfly_join <- left_join(states, butterfly_location_summary, by = c("abrev"="state_province"))

#Second join  Inner join to get the location data. Call the resulting dataframe USA_map_joined.
USA_map_joined <- inner_join(USA_map, butterfly_join, by= c("ID"="lower_state"))
...
# Make map here.
ggplot() +
  geom_sf(data = USA_map_joined, aes(fill = proportion)) +
  scale_fill_distiller(palette = "YlOrRd", direction = 1) +
  labs(
     title = "Butterfly Sightings by State"
   )+
...
USA_map_joined|>
  ggplot()+
  geom_sf(aes(fill=total_sightings_state)) +
...
US_tot_pop <- inner_join(USA_map_joined, US_pop, by = c("ID" ="NAME"))
 ...
 US_tot_pop <- US_tot_pop |>
   mutate(proportion_population = total_sightings_2022 / estimate)

# Make your new map here. 
ggplot() +
   geom_sf(data = US_tot_pop, aes(fill = proportion_population), color = "black") +
  scale_fill_distiller(palette = "YlOrRd", direction = 1) +
   labs(
     title = "Butterfly Sightings Proportion by State Population",
     caption = "Data source: Monarch Watch Tag Recoveries - Central Mexico"
   ) +
...
**Question: What does the above map show?** The above map shows where the proportion of butterfly sightings was highest in relation to the population size of the state. Here we see that Vermont and South Dakota have some of the highest proportions of butterfly sightings in relation to population size.
...
leaflet() |>
  addTiles() |>
  addMarkers(data= na.omit(butterfly_location_data_8min), 
             lat = ~lat,
             lng = ~long,
             icon = butterfly_icon
             ) |>
  addCircleMarkers(data = na.omit(Mexico_locations),
                   lat = ~lat,
                   lng = ~long,
                   popup = ~as.character(location))
  
#Please note: it did not have lat/longitude values available for three of the Mexico locations, which is why there are four dots shown
```
